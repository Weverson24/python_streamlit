...
total_compra = dados.groupby('Local da compra')[['Preço']].sum()
total_compra = dados.drop_duplicates(
    subset = 'Local da compra')[['Local da compra', 'lat', 'lon']].merge(
        total_compra, left_on = 'Local da compra', right_index = True
        ).sort_values('Preço', ascending = False)


fig_mapa_receita = px.scatter_geo(total_compra,
                                   lat = 'lat',
                                   lon = 'lon',
                                   scope = 'south america',
                                   size = 'Preço',
...
fig_receita_estados = px.bar(total_compra.head(),
                                            x = 'Local da compra',
                                            y = 'Preço',
                                            text_auto = True,
                                            title = 'Top estados')
...
total_mensal = dados.set_index('Data da Compra').groupby(pd.Grouper(freq='M'))['Preço'].sum().reset_index()
total_mensal['Ano'] = total_mensal['Data da Compra'].dt.year
total_mensal['Mês'] = total_mensal['Data da Compra'].dt.month_name()


fig_receita_mensal = px.line(total_mensal,
                                x = 'Mês',
                                y = 'Preço',
                                markers = True,
                                range_y = (0, total_mensal.max()),
                                color='Ano',
                                line_dash = 'Ano',
...
total_categoria = dados.groupby('Categoria do Produto')[['Preço']].sum().sort_values('Preço', ascending=False)



fig_receita_categorias = px.bar(total_categoria,
                                    text_auto = True,
...
with tab1:    
    ...
    with col1:
        st.metric('Receitas',formatar_numero(dados['Preço'].sum(),'R$'))
...
with tab3:    
    ...
    with col1:
        st.metric('Receitas',formatar_numero(dados['Preço'].sum(),'R$'))
        fig_receita_vededores = px.bar(vendedores[['sum']].sort_values('sum',ascending=False).head(qtd_vendedores),
                                       x='sum',
    ...
    with col2:
        st.metric('Quantidades de vendas',formatar_numero(dados.shape[0]))
        fig_vendas_vededores = px.bar(vendedores[['count']].sort_values('count',ascending=False).head(qtd_vendedores),
                                       x='count',
